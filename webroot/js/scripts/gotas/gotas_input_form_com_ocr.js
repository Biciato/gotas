/**
 * @file webroot/js/scripts/gotas/gotas_input_form_with_ocr.js
...
$(document).ready(function () {
    ...
    /**
     *  Carrega todos os parâmetros disponíveis se estado é diferente de MG
     */
    if ($("#estado_funcionario").val() != 'MG') {
        ...
        $.ajax({
            ...
            if (result.gotas !== undefined && result.gotas.length > 0) {
                ...
                $(".gotas-instascan-manual-insert #list_parametros").append($("<option>"));
    ...
    $(".gotas-instascan-manual-insert #chave_nfe").mask('9999 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999');

    // Lista de seleção de parâmetros
    $(".gotas-instascan-manual-insert #list_parametros").prop('disabled', true);
    ...
    $(".gotas-instascan-manual-insert #quantidade_input").on('blur', function () {
    ...
    $(".gotas-instascan-manual-insert .save-receipt-button").prop('disabled', true);
    ...
    $(".gotas-instascan-manual-insert .call-modal-how-it-works").on('click', function () {
    ...
    /**
     * Inicializa os botões de remover da tabela de itens a gravar
     */
    var initializeSelectClicks = function () {
        $(".gotas-instascan-manual-insert .select-button").on('click', function () {
    ...
    $(".gotas-instascan-manual-insert #chave_nfe").on('keydown', function (data) {
        if (!isNaN(data.key)) {
            return;
        }
    });
    ...
    var checkTaxCouponRepeated = function (function_execute) {
        var chave_nfe = $(".gotas-instascan-manual-insert #chave_nfe").val();
        ...
        if (chave_nfe_original.length < 54) {
            $(".gotas-instascan-manual-insert #list_parametros").attr('disabled', true);
        } else {
            ...
            $.ajax({
                ...
                if (result.found) {
                    ...
                    $(".gotas-instascan-manual-insert .save-receipt-button").attr('disabled', true);
                    $(".gotas-instascan-manual-insert #list_parametros").val(null);
                    $(".gotas-instascan-manual-insert #list_parametros").attr('disabled', true);
                    $(".gotas-instascan-manual-insert #list_parametros").change();

                } else {
                    $(".gotas-instascan-manual-insert #list_parametros").attr('disabled', false);

                    if (arrayParametrosGravar.get().length > 0) {
                        $(".gotas-instascan-manual-insert .save-receipt-button").attr('disabled', false);
     ...
     */
    $(".gotas-instascan-manual-insert #chave_nfe").on('keyup', function (data) {
    ...
    var enableSelectParameter = function () {
        if ($(".gotas-instascan-manual-insert #list_parametros").val() !== "" && $(".gotas-instascan-manual-insert #chave_nfe").val().length == 54) {
            $(".gotas-instascan-manual-insert .select-parameter").prop('disabled', false);

        } else {
            $(".gotas-instascan-manual-insert .select-parameter").prop('disabled', true);
        }
    }

    // Configura botão de adicionar registro

    $(".gotas-instascan-manual-insert #list_parametros").on('change', function () {
        var key = $("#list_parametros").val();

        var parameter = arrayGotas.findByKey(key);
        parameter == undefined ? $(".gotas-instascan-manual-insert #quantidade_input").prop('disabled', true) : $(".gotas-instascan-manual-insert #quantidade_input").prop('disabled', false);
    ...
    $(".gotas-instascan-manual-insert #quantidade_input").on('keyup', function () {
        if ($(this).val() != "") {
            $(".gotas-instascan-manual-insert .add-parameter").prop('disabled', false);
        } else {
            $(".gotas-instascan-manual-insert .add-parameter").prop('disabled', true);
        }
    });
    ...
    $(".gotas-instascan-manual-insert .add-parameter").on('click', function () {

        var key = $(".gotas-instascan-manual-insert #list_parametros").val();
        ...
        var itemToAdd = {
            ...
            quantidade_multiplicador: parseFloat($(".gotas-instascan-manual-insert #quantidade_input").val())
        };
    ...
    var updateTableParametrosGravar = function () {
        $(".gotas-instascan-manual-insert .gotas-products-table >tbody").html('');

        if (arrayParametrosGravar.get().length > 0) {
            $(".gotas-instascan-manual-insert .save-receipt-button").prop('disabled', false);
        } else {
            $(".gotas-instascan-manual-insert .save-receipt-button").prop('disabled', true);
        }
        ...
        $.each(arrayParametrosGravar.get(), function (index, value) {
            ...
            $(".gotas-instascan-manual-insert .gotas-products-table ").append(html);
        });
    ...
    var saveReceipt = function () {
        ...
        $.ajax({
            ...
            if (result.success) {
                ...
                $(".gotas-instascan-manual-insert #chave_nfe").val(null);
                $(".gotas-instascan-manual-insert #list_parametros").val(0);
                $(".gotas-instascan-manual-insert #list_parametros").change();

                // $("#nome_parametro_input").val(null);
                $(".gotas-instascan-manual-insert #quantidade_input").val(null);
                ...
                // TODO: fazer tela informando que houve erro ao processar, estudar como fazer isso
