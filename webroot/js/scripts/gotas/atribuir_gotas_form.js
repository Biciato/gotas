...
$(document).ready(function () {
    ...
    var checkURLConsistency = function (url) {
        ...
        $.each(arrayConsistency, function (index, value) {
            ...
            if (!value.isOptional) {
                ...
                if (keyIndex == -1) {
                    errorType = "Campo <strong>" + value.key + "</strong> do QR Code deve ser informado";
                } else {
                    ...
                    // captura conteúdo
                    value.content = url.substr(value.index, indexEnd - value.index);
                    ...
                    if (containsBlank == -1) {
                        ...
                        if (value.fixedSize) {
                            if (length != value.size) {
                                errorType = "Campo <strong>" + value.key + "</strong> do QR Code deve conter " + value.size + " bytes";

                            }
                        }

                    } else {
                        errorType = "Campo <strong>" + value.key + "</strong> contêm espaço em branco.";
                    }
                }
        ...
        if (arrayErrors.length > 0) {
            ...
            callModalError(`O QR Code informado não está gerado conforme os padrões pré- estabelecidos da SEFAZ. <br />
    ...
    $(".video-gotas-capture-container .video-gotas-snapshot").on('click', function () {
